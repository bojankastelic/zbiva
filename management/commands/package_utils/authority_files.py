...
from zbiva.models.zbiva_utils import send_mail_to_admins
                ...
                for row in rows:
                    try:
                        if 'ADDITIONAL' in row:
                            ...
                            if row_valuetype.startswith('label:'):
                                ll_list = row_valuetype.split(':')
                                language = ll_list[1]
                                concept.addvalue({'value':row[u'VALUE'], 'language': language, 'type': 'prefLabel', 'category': 'label'})
                            else:
