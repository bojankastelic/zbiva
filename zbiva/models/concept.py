...
class Concept(object):
    ...
    def get_e55_domain(self, entitytypeid, lang):
        """
        ...
        WITH RECURSIVE children AS (
            SELECT d.conceptidfrom, d.conceptidto, c2.value, c2.valueid as valueid, c.value as valueto, c.valueid as valueidto, c.valuetype as vtype, 1 AS depth, array[d.conceptidto] AS conceptpath, array[c.valueid] AS idpath        ---|NonRecursive Part
                ...
                and ((c.languageid = '{1}'
                and c.valuetype in ('prefLabel')) or
                (c.valuetype in ('sortorder', 'collector')))
                ...
                and ((v.languageid = '{1}'
                and v.valuetype in ('prefLabel')) or
                (v.valuetype in ('sortorder', 'collector')))
                and (d.relationtype = 'member' or d.relationtype = 'hasTopConcept')
        ...
        """.format(entitytype.conceptid_id, lang)
    ...
    @staticmethod
    def get_time_filter_data(lang):
        important_dates = []
        for date_search_entity_type in settings.DATE_SEARCH_ENTITY_TYPES:
            important_dates = important_dates + Concept().get_e55_domain(date_search_entity_type, lang)
